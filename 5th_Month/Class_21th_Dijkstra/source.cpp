	...
	void calculate(int start) {
		...
		for (int i = 0; i < this->SIZE; i++) {
			if (this->visited[i] == true) { continue; }
				...
				if (this->distance[i] >= INFINITY) { this->distance[i] = this->weights[target][i]; }
		...
		for (int i = 0; i < this->SIZE; i++) {
			...
			if (this->visited[i] == true) { continue; }
				...
				if (min > this->distance[i]) { min = this->distance[i]; }
			}
		}
