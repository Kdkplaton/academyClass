...
public:
	...
	void find_lowestDist() {
		this->sort_edges();
		int count = 1;		// 간선 숫자 체크용 카운터 ('노드의 수 - 1' 보다 클 수 없음)

		cout << "cost: ";
		for (int i = 0; i < this->edges.size(); i++) {
			// this->show_visited();
			for (int j = 1; j < this->SIZE; j++) {
				if (this->visited[j] == false) { break; }
				else if (j + 1 == this->SIZE) {
					cout << "   Find Ended!" << endl;
					cout << "result: " << this->cost << endl;
					return;
				}
			}

			if (count > this->SIZE - 2) { cout << "  Error!! N-1 Reached!!"; break; }

			int node1 = this->edges[i].get_start();
			int node2 = this->edges[i].get_end();
			if (node1 > node2) {
				int temp = node1;
				node1 = node2;
				node2 = temp;
			...
			if (this->parents[node1] == this->parents[node2]) { continue; }
			else {
				if (this->parents[node1] > this->parents[node2]) { this->parents[node1] = this->parents[node2]; }
				else { this->parents[node2] = this->parents[node1]; }
			}

			if (i == 0) { cout << this->edges[i].get_dist(); }
			else { cout << " + " << this->edges[i].get_dist(); }

			this->visited[node1] = true;
			this->visited[node2] = true;


			cost += this->edges[i].get_dist();
			count++;
		}
		cout << endl;
	...
	void show_visited() {
		cout << endl;
		for (int i = 1; i < this->SIZE; i++) { cout << i << " "; }
		cout << endl;
		for (int i = 1; i < this->SIZE; i++) { cout << this->visited[i] << " "; }
		cout << endl;
	}
};
