...
void merge_sort(int list[], int start, int end) {
	...
	if (len > 2) {
		...
		cout << "p1: " << p1 << " p2: " << p2 << " end: " << end << endl;
		for (int i = 0; i < len; i++) {
			if (p1 <= mid || p2 <= end) {
				if (p1 > mid) { temp[i] = list[p2++]; }
				else if (p2 > end) { temp[i] = list[p1++]; }
				else if (list[p1] <= list[p2]) { temp[i] = list[p1++]; }
				else { temp[i] = list[p2++]; }
			}
		}
...
#pragma region 병합 정렬 (Merge Sort)
	...
	const int SIZE = 10;
	int datas[SIZE];
	for (int i = 0; i < SIZE; i++) { datas[i] = rand() % 100 + 1; }
