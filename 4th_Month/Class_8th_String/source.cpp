...
public:
	...
	int Find(const char* input) {
		int res = -1;		// 결과값 : 조회 실패시 -1 반환
		int length = getLen(input);		// 입력된 문자열의 길이

		for (int i = 0; i < (this->size - length); i++) {		// 총 길이에서 검색할 문자열의 길이 만큼 빼고 반복
			if (this->string[i] == input[0]) {					// 시작점(으로 추정되는 지점) 발견시
				res = i;										// 발견 지점에 시작점 등록
				for (int j = 0; j < length; j++) {				// 확인절차 진행
					if (this->string[i + j] == input[j]) {}		// 해당 지점부터 비교
					else { res = -1; break; }					// 일치하지 않는다면 발견지점 -1(미발견) 설정
				}
			}
			else { continue; }		// 시작추정지점 발견할때까지 통과
		}

		return res;		// 반환값은 index값, 없으면 -1 반환
	}
...
#pragma region 문자열(String)
	...
	str1.printStr();
	cout << "\"el\" 검색결과: " << str1.Find("el") << endl;
	cout << "\"rl\" 검색결과: " << str1.Find("rl") << endl;
	cout << "\"ld\" 검색결과: " << str1.Find("ld") << endl;
	cout << "\"hel\" 검색결과: " << str1.Find("hel") << endl;
